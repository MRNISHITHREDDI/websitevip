...
const TrxPrediction: React.FC<PredictionPageProps> = ({ timeOption }) => {
  ...
  useEffect(() => {
    ...
    if (!isFullyVerified) {
      console.log(
        "TRX: Account not fully verified, showing verification modal",
      );
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const { currentPrediction, results, hash } =
        await fetchTrxData(timeOption);
          ...
          setPreviousPredictions((prev) => {
            ...
            if (latestResultIndex !== -1) {
              ...
              const actualBigSmall =
                resultNumber >= 5 ? ("BIG" as const) : ("SMALL" as const);
              ...
              console.log(
                `TRX: Updated prediction for period ${latestResult.periodNumber} with result ${resultNumber}`,
              );
            } else {
              // We don't have a prediction for this result period, but we should still record it
              // This ensures we don't miss any results
              console.log(
                `TRX: No existing prediction found for period ${latestResult.periodNumber}, recording result only`,
              );
              const resultNumber = parseInt(latestResult.result.toString());
            ...
            if (!currentPredictionExists) {
              console.log(
                `TRX: Adding new prediction for period ${currentPrediction.periodNumber}`,
              );
      ...
      const { currentPrediction, results, hash } =
        generateMockTrxData(timeOption);
              ...
              <div className="flex flex-col items-center relative">
                ...
                >
                  <span className="uppercase text-xs tracking-wide">
                    Use 3X Investment and Earn Big With TRX Hash
                  </span>
                </motion.div>
              </div>
            </motion.div>
